...
pub fn run() {
    // Audio is decoded and played natively in this process (see audio.rs)
    // so output originates from the wavejack PID — capturable by per-window
    // screenshare tools and reusable as a PCM source for room broadcast.
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            audio_player.spawn_emitter(app.handle().clone());
